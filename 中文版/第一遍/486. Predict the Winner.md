...
插播一个消息，我前不久发现leetcode上支持的语言已经非常多了，之前一直没发现（因为要下拉才能看到），另外我在换语言的时候遇到一些障碍，然后在google上搜，最后又有个新的发现，其实你AC一次之后，其他的代码是可以看到的（以前是这样的），现在更方便，直接就可以点开discuss和top solutions。在leetcode上直接看各种大神的优秀代码岂不是更爽？
